...
class InferenceModel(model.SockeyeModel):
    ...
    def initialize(self, max_input_length: int, max_ctx_input_length, get_max_output_length_function: Callable):
        """
    ...
    def _get_encoder_module(self) -> Tuple[mx.mod.BucketingModule, Tuple[int, int]]:
        """
        ...
        def sym_gen(bucket_key: Tuple[int, int]):
            ...
            source_seq_len, ctx_source_seq_len = bucket_key
            ...
            (ctx_source_embed,
             ...
             ctx_source_embed_seq_len) = self.ctx_embedding_source.encode(ctx_source, ctx_source_length, ctx_source_seq_len)
        ...
        default_bucket_key = (self.max_input_length, self.max_ctx_input_length)
    ...
    def _get_decoder_module(self) -> Tuple[mx.mod.BucketingModule, Tuple[int, int, int]]:
        ...
        def sym_gen(bucket_key: Tuple[int, int, int]):
            ...
            if self.model_type == "ctx_enc":
                ctx_source_seq_len = source_seq_len
            ...
            ctx_source_embed_seq_len = self.embedding_source.get_encoded_seq_len(ctx_source_seq_len)
        ...
        default_bucket_key = (self.max_input_length, self.max_ctx_input_length, self.get_max_output_length(self.max_input_length))
        module = mx.mod.BucketingModule(sym_gen=sym_gen,
                                        default_bucket_key=default_bucket_key,
                                        context=self.context)
        return module, default_bucket_key

    def _get_encoder_data_shapes(self, bucket_key: Tuple[int, int]) -> List[mx.io.DataDesc]:
        """
    ...
    def _get_decoder_data_shapes(self, bucket_key: Tuple[int, int, int]) -> List[mx.io.DataDesc]:
        ...
        return self.decoder_data_shapes_cache.setdefault(
            ...
            self.decoder.state_shapes(self.batch_size * self.beam_size,
                                      ...
                                      self.encoder.get_encoded_seq_len(ctx_source_max_length),
                                      self.encoder.get_num_hidden()))

    def run_encoder(self,
                    source: mx.nd.NDArray,
                    ctx_source: mx.nd.NDArray,
                    source_max_length: int, ctx_source_max_length: int) -> 'ModelState':
        """
        ...
        """
        batch = mx.io.DataBatch(data=[source, ctx_source],
                                label=None,
                                bucket_key=(source_max_length, ctx_source_max_length),
                                provide_data=self._get_encoder_data_shapes((source_max_length, ctx_source_max_length)))
...
def load_models(context: mx.context.Context,
    ...
    for model in models:
        model.initialize(max_input_len, max_ctx_input_len, get_max_output_length)
...
class Translator:
    ...
    def __init__(self,
        ...
        if bucket_source_width > 0:
            ...
            self.buckets_source = data_io.define_ctx_buckets(self.max_input_length, step=bucket_source_width, ctx_max_seq_len=self.max_ctx_input_length)
        else:
    ...
    @staticmethod
    def ctx_make_input(sentence_id: int, sentence: str) -> TranslatorInput:
        """
        Returns TranslatorInput from input_string
    ...
    def translate(self, trans_inputs: List[TranslatorInput]) -> List[TranslatorOutput]:
        ...
        for input_idx, trans_input in enumerate(trans_inputs):
            ...
            elif len(trans_input.tokens[0]) > self.max_input_length:
                logger.debug(
                    "Input %d has length (%d) that exceeds max input length (%d). Splitting into chunks of size %d.",
                    trans_input.id, len(trans_input.tokens[0]), self.buckets_source[-1], self.max_input_length)
                token_chunks = utils.ctx_chunks(trans_input.tokens, self.max_input_length)
    ...
    def _get_inference_input(self, sequences: List[List[str]]) -> Tuple[mx.nd.NDArray, int]:
        ...
        for tokens in sequences:
            if len(tokens[0]) > max_tok:
                max_tok = len(tokens[0])
            if len(tokens[1]) > max_ctx_tok:
                max_ctx_tok = len(tokens[1])
        bucket_key = data_io.get_ctx_bucket(max_tok, max_ctx_tok, self.buckets_source)

        utils.check_condition(C.PAD_ID == 0, "pad id should be 0")
        source = mx.nd.zeros((len(sequences), bucket_key[0]))
        ctx_source = mx.nd.zeros((len(sequences), bucket_key[1]))

        for j, tokens in enumerate(sequences):
            source[j, :len(tokens[0])] = data_io.tokens2ids(tokens[0], self.vocab_source)
            if len(tokens[1]) > 0:
                ctx_source[j, :len(tokens[1])] = data_io.tokens2ids(tokens[1], self.vocab_source)

        return source, ctx_source, bucket_key
    ...
    def translate_nd(self,
        ...
        """
        return self._get_best_from_beam(*self._beam_search(source, ctx_source, source_length))

    def _encode(self, sources: mx.nd.NDArray, ctx_sources: mx.nd.NDArray, source_length: int, ctx_source_length: int) -> List[ModelState]:
        """
        ...
        """
        return [model.run_encoder(sources, ctx_sources, source_length, ctx_source_length) for model in self.models]
    ...
    def _beam_search(self,
        ...
        encoded_source_length = self.models[0].encoder.get_encoded_seq_len(source_length[0])
        utils.check_condition(all(encoded_source_length ==
                                  model.encoder.get_encoded_seq_len(source_length[0]) for model in self.models),
                              "Models must agree on encoded sequence length")
        # Maximum output length
        max_output_length = self.models[0].get_max_output_length(source_length[0])
        ...
        model_states = self._encode(source, ctx_source, source_length[0], source_length[1])
