...
def define_ctx_buckets(max_seq_len: int, step=10, ctx_max_seq_len=None) -> List[Tuple]:

    if ctx_max_seq_len is None:
        ctx_max_seq_len = max_seq_len

    buckets = [(bucket_len, ctx_bucket_len) for bucket_len in range(step, max_seq_len + step, step)
               for ctx_bucket_len in range(step, ctx_max_seq_len + step, step)]
...
def define_parallel_buckets(max_seq_len_source: int,
                            ...
                            bucket_width: int = 10,
                            length_ratio: float = 1.0) -> List[Tuple[int, int, int]]:
    """
    ...
    ctx_source_buckets = define_buckets(max_seq_len_ctx_source, step=source_step_size)
    ...
    parallel_buckets = list(zip(source_buckets, target_buckets, ctx_source_buckets))
...
class BucketBatchSize:
    ...
    def __init__(self, bucket: Tuple[int, int, int], batch_size: int, average_words_per_batch: float) -> None:
        self.bucket = bucket
...
def define_bucket_batch_sizes(buckets: List[Tuple[int, int, int]],
    ...
    for buck_idx, bucket in enumerate(buckets):
        ...
        largest_total_num_words = max(largest_total_num_words, batch_size_seq * max(*bucket[:2]))
    ...
    if batch_by_words:
        padded_seq_len = max(*buckets[-1][:-1])
...
def calculate_length_statistics(source_sentences: Iterable[List[Any]],
                                ...
                                max_seq_len_target: int,
                                max_seq_len_ctx_source: int) -> 'LengthStatistics':
    """
    ...
    for target, source in zip(target_sentences, source_sentences):
        ...
        ctx_source_len = len(source[1])
        target_len = len(target[0])
        if source_len > max_seq_len_source or target_len > max_seq_len_target or ctx_source_len > max_seq_len_ctx_source:
            continue
...
class DataStatisticsAccumulator:
    ...
    def sequence_pair(self,
                      source: Tuple[List[int], List[int]],
        ...
        self.num_unks_source += source[0].count(self.unk_id_source)
        self.num_unks_target += target[0].count(self.unk_id_target)
...
def shard_data(source_fname: str,
    ...
    logging.error("Context-aware implementation does not currently support sharding data.")
...
class RawParallelDatasetLoader:
    ...
    def load(self,
        ...
        data_source = [np.full((num_samples, source_len), self.pad_id, dtype=self.dtype)
                       for (source_len, target_len, ctx_source_len), num_samples in zip(self.buckets, num_samples_per_bucket)]
        ...
        for source, target in zip(source_sentences, target_sentences):
            ...
            target_len = len(target[0])
            ctx_source_len = len(source[1])
            buck_index, buck = get_parallel_bucket(self.buckets, source_len, target_len, ctx_source_len)
            if buck is None:
            ...
            data_source[buck_index][sample_index, :source_len] = source[0]

            data_ctx_source[buck_index][sample_index, :ctx_source_len] = source[1]

            data_target[buck_index][sample_index, :target_len] = target[0]
            ...
            data_label[buck_index][sample_index, :target_len] = target[0][1:] + [self.eos_id]
        ...
        return ParallelDataSet(data_source, data_ctx_source, data_target, data_label)
...
def prepare_data(source: str, target: str,
    ...
    logger.error('Context-aware implementation does not currently support preparing data.')
...
def get_data_statistics(source_sentences: Iterable[Tuple[List[int],List[int]]],
                        target_sentences: Iterable[Tuple[List[int],List[int]]],
                        buckets: List[Tuple[int, int, int]],
                        length_ratio_mean: float,
                        length_ratio_std: float,
    ...
    for source, target in zip(source_sentences, target_sentences):
        buck_idx, buck = get_parallel_bucket(buckets, len(source[0]), len(target[0]), len(source[1]))
        data_stats_accumulator.sequence_pair(source, target, buck_idx)
...
def get_validation_data_iter(data_loader: RawParallelDatasetLoader,
    ...
    validation_length_statistics = analyze_sequence_lengths(validation_source, validation_target,
                                                            ...
                                                            max_seq_len_source, max_seq_len_target, max_seq_len_ctx_source)
...
def get_prepared_data_iters(prepared_data_dir: str,
                            ...
                            batch_by_words: bool,
                            batch_num_devices: int,
                            fill_up: str) -> Tuple['BaseParallelSampleIter', 'BaseParallelSampleIter', 'DataConfig', vocab.Vocab, vocab.Vocab]:
    logger.info("===============================")
...
def get_training_data_iters(source: str, target: str,
    ...
    # define buckets
    buckets = define_parallel_buckets(max_seq_len_source, max_seq_len_target, max_seq_len_ctx_source, bucket_width,
                                      length_statistics.length_ratio_mean) if bucketing else [
        (max_seq_len_source, max_seq_len_target)]
...
def ctxtokens2ids(tokens: Iterable[str], vocab: Dict[str, int]) -> List[int]:
    """
    Returns sequence of integer ids given a sequence of tokens and vocab.
...
class SequenceReader(Iterator):
    ...
    def __iter__(self):
        ...
        self._iter = read_ctx_content(self.path, self.limit)
    ...
    def __next__(self):
        ...
        if self.vocab is not None:
            sequence = [tokens2ids(tokens[0], self.vocab), tokens2ids(tokens[1], self.vocab)]
        else:
            sequence = strids2ids(tokens)

        check_condition(bool(sequence[0]), "Empty sequence in file %s" % self.path)

        if vocab is not None and self.add_bos:
            sequence[0].insert(0, self.vocab[C.BOS_SYMBOL])
            sequence[1].insert(0, self.vocab[C.BOS_SYMBOL])
...
def get_parallel_bucket(buckets: List[Tuple[int, int, int]],
                        length_source: int,
                        length_target: int,
                        length_ctx_source: int) -> Optional[Tuple[int, Tuple[int, int, int]]]:
    """
    ...
    for j, (source_bkt, target_bkt, ctx_source_bkt) in enumerate(buckets):
        if source_bkt >= length_source and target_bkt >= length_target and ctx_source_bkt >= length_ctx_source:
            bucket = j, (source_bkt, target_bkt, ctx_source_bkt)
            break
    return bucket
...
class ParallelDataSet(Sized):
    ...
    def save(self, fname: str):
        ...
        """
        mx.nd.save(fname, self.source + self.ctx_source + self.target + self.label)
    ...
    def load(fname: str) -> 'ParallelDataSet':
        ...
        target = data[2 * n:3 * n]
        label = data[3 * n:]
        assert len(source) == len(target) == len(label) == len(ctx_source)
        return ParallelDataSet(source, ctx_source, target, label)
        ...
        for bucket_idx in range(len(self)):
            ...
            if num_samples % bucket_batch_size != 0:
                if fill_up == 'replicate':
                    ...
                    ctx_source[bucket_idx] = mx.nd.concat(bucket_ctx_source, bucket_ctx_source.take(random_indices), dim=0)
...
class BaseParallelSampleIter(mx.io.DataIter, ABC):
    ...
    def __init__(self,
        ...
        self.data_names = [self.source_data_name, self.ctx_source_data_name, self.target_data_name]
...
class ParallelSampleIter(BaseParallelSampleIter):
    ...
    def __init__(self,
        ...
        self.data = ParallelDataSet(list(data.source), list(data.ctx_source), list(data.target), list(data.label))
