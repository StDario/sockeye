...
from sockeye.utils import check_condition, grouper_ctx
...
def read_and_translate(translator: sockeye.inference.Translator, output_handler: sockeye.output_handler.OutputHandler,
    ...
    for chunk in grouper_ctx(source_data, chunk_size):
...
def translate(output_handler: sockeye.output_handler.OutputHandler, source_data: Iterable[str],
    ...
    tic = time.time()
    trans_inputs = [translator.ctx_make_input(i, line) for i, line in enumerate(source_data, chunk_id + 1)]
